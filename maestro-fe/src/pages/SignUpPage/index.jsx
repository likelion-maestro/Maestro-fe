...
const SignUpPage = () => {
  ...
  const onClickSignUp = () => {
    const data = {
      ...
      nickname: name,
      password: password,
      repass: passwordCheck,
    };

    axios
      .post("http://" + process.env.REACT_APP_API + "/user/signUp", data)
      .then((res) => {
  ...
  return (
    <Layout>
      ...
      />
      <ButtonWrapper>
        <Button small disabled={notAllow} onClick={onClickSignUp}>
          <ButtonText onClick={onClickSignUp}>
            입력한 정보로 회원가입
          </ButtonText>
        </Button>
      </ButtonWrapper>
